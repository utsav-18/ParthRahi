    ...
    <header className="relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 md:px-16 pt-28 md:pt-36 pb-10 md:pb-14">
        <nav className="text-xs text-amber-200/60 mb-5">
          <Link to="/events" className="hover:text-amber-100">Yatras</Link>
        ...
        <div className="max-w-2xl">
          ...
          <div className="mt-6 flex flex-wrap items-end gap-x-8 gap-y-4">
            <div>
              <p className="text-[10px] uppercase tracking-widest text-amber-200/60">From</p>
              ...
              <p className="text-[11px] text-amber-200/50">
                {yatra.price?.unit || "per person"}
                {yatra.price?.advanceAmount ? ` · reserve with ${formatCurrency(yatra.price.advanceAmount, yatra.price.currency)}` : ""}
              </p>
            </div>
            <div className="text-sm text-amber-100/80">
              <p>📍 {yatra.startingPoint}</p>
              <p className="mt-1">🗓 Next departure: {dep ? formatDate(dep) : "to be announced"}</p>
            </div>
          </div>
