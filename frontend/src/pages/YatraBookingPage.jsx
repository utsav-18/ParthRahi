...
export default function YatraBookingPage() {
  ...
  useDocumentMeta({ title: yatra ? `${t("booking.book")} — ${yatra.title}` : t("booking.bookYourSeatTitle") });
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (active) api.get(`/api/bookings/availability/${slug}`).then((res) => setAvailability(res.data)).catch(() => {});
    }, 15000);
    return () => {
      active = false;
      clearInterval(timer);
  ...
  useEffect(() => {
    ...
    setForm((current) => ({
      ...current,
      travelerName: current.travelerName || user.name || "",
      phone: current.phone || user.phone || "",
      email: current.email || user.email || "",
    }));
    if (selectedSeats.length) {
      api
        .get(`/api/bookings/availability/${slug}`)
        .then((res) => {
          ...
          if (unavailable.length) {
            ...
            setServerError(t("booking.seatNoLongerAvailable", { seats: unavailable.join(", ") }));
          }
        })
        .catch(() => {});
    }
  ...
  useEffect(() => {
    ...
    const updateRemaining = () => {
      ...
      if (seconds === 0) {
        ...
        setSelectedSeats([]);
        setStep(1);
        setServerError(t("booking.reservationExpiredBody"));
  ...
  const validateStep1 = () => {
    if (!user) {
      setServerError(t("booking.pleaseLogin"));
      openLogin();
      return false;
    }
    const next = {};
  ...
  const createSeatHold = async () => {
    const hold = await api.post("/api/bookings/lock", {
      yatraSlug: slug,
    ...
    setReservationExpiresAt(Date.parse(expiresAt));
      ...
      const payment = new window.Razorpay({
        ...
        handler: async (response) => {
          try {
            ...
            setActiveHold(null);
            setReservationExpiresAt(null);
            setConfirmation(verified.data.booking);
            setStep(3);
            window.scrollTo({ top: 0, behavior: "smooth" });
          } catch (error) {
            setServerError(error.message);
          ...
          {step === 2 && (
            <div className="max-w-2xl mx-auto w-full space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={beginPayment} disabled={submitting || (remainingSeconds != null && remainingSeconds <= 0)} className={`${btnAccent} flex-1`}>
