...
const LoginModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    let interval;
    if (resendTimer > 0) {
      interval = setInterval(() => setResendTimer(s => s - 1), 1000);
    }
  ...
  const handleSignup = async (e) => {
    e.preventDefault();
    setError('');
    if (!phone.trim()) return setError(t('auth.errPhoneRequired'));
  ...
  const handleVerifyOtp = async (e) => {
    e.preventDefault();
    setError('');
    if (otp.length !== 6) return setError(t('auth.errOtpLength'));
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      setResendTimer(60);
      setMessage(t('auth.resetOtpSentGeneric'));
    } catch (err) {
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-semibold text-white">
            ...
            {view === 'verify-otp' && t('auth.verifyTitle')}
            {view === 'forgot-password' && t('auth.forgotTitle')}
            {view === 'reset-password' && t('auth.resetTitle')}
          </h2>
        </div>
        ...
        {view === 'login' && (
          <>
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <input type="email" placeholder={t('auth.emailPlaceholder')} required value={email} onChange={e => setEmail(e.target.value)} className={inputClass} />
              </div>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} placeholder={t('auth.passwordPlaceholder')} required value={password} onChange={e => setPassword(e.target.value)} className={inputClass} />
            ...
            <div className="my-6 flex items-center gap-3">
              ...
              <span className="text-xs text-slate-500 uppercase tracking-wider">{t('auth.or')}</span>
              <div className="flex-1 h-px bg-slate-700/50"></div>
            </div>
        ...
        {view === 'signup' && (
          <>
            <form onSubmit={handleSignup} className="space-y-4">
              <div>
                <input type="text" placeholder={t('auth.namePlaceholder')} required value={name} onChange={e => setName(e.target.value)} className={inputClass} />
              </div>
              <div>
                <input type="email" placeholder={t('auth.emailPlaceholder')} required value={email} onChange={e => setEmail(e.target.value)} className={inputClass} />
              </div>
              <div>
                <input type="tel" placeholder={t('auth.phonePlaceholder')} required value={phone} onChange={e => setPhone(e.target.value)} className={inputClass} />
              </div>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} placeholder={t('auth.signupPasswordPlaceholder')} required value={password} onChange={e => setPassword(e.target.value)} className={inputClass} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-slate-400 hover:text-white cursor-pointer">
                  {showPassword ? t('auth.hide') : t('auth.show')}
                </button>
              </div>
              <div className="relative">
                <input type={showConfirmPassword ? "text" : "password"} placeholder={t('auth.confirmPasswordPlaceholder')} required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className={inputClass} />
        ...
        {view === 'forgot-password' && (
          <form onSubmit={handleForgotPassword} className="space-y-6">
            <p className="text-sm text-slate-400 text-center mb-2">{t('auth.forgotDesc')}</p>
            <div>
              <input type="email" placeholder={t('auth.emailPlaceholder')} required value={email} onChange={e => setEmail(e.target.value)} className={inputClass} />
            </div>
            <button type="submit" disabled={loading} className={btnClass}>
        ...
        {view === 'reset-password' && (
          <form onSubmit={handleResetPassword} className="space-y-4">
            <p className="text-sm text-slate-400 text-center mb-4">{t('auth.resetDesc')}</p>
            <div>
              <input
                type="text"
                maxLength={6}
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} placeholder={t('auth.newPasswordPlaceholder')} required value={password} onChange={e => setPassword(e.target.value)} className={inputClass} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-slate-400 hover:text-white cursor-pointer">
                {showPassword ? t('auth.hide') : t('auth.show')}
              </button>
            </div>
            <div className="relative">
              <input type={showConfirmPassword ? "text" : "password"} placeholder={t('auth.confirmNewPasswordPlaceholder')} required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className={inputClass} />
              <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-slate-400 hover:text-white cursor-pointer">
                {showConfirmPassword ? t('auth.hide') : t('auth.show')}
              </button>
            </div>
